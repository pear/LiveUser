...
class LiveUser_Auth_MDB2 extends LiveUser_Auth_Common
{
     ...
     * @return void
     */
    function LiveUser_Auth_MDB2(&$connectOptions)
    {
    ...
    function userExists($checkHandle = false, $checkPW = false)
    {
        ...
        if ($checkHandle !== false && $checkPW === false) {
            ...
            $sql .= 'WHERE '  . $this->authTableCols['required']['handle']['name'] . '='
                . $this->dbc->quote($checkHandle, $this->authTableCols['required']['handle']['type']);
            ...
            $sql .= 'WHERE '  . $this->authTableCols['required']['passwd']['name'] . '='
                . $this->dbc->quote($this->encryptPW($checkPW), $this->authTableCols['required']['passwd']['type']);
        } else {
